...
return jQuery;});$(function(){function replaceTimeTag(inTheFuture){return function(){var self=$(this);var date=self.attr('datetime')||self.data('date');self.attr('title',self.text()).text(moment(date).fromNow(inTheFuture));};}
function updateTimes(){$('time.ago').each(replaceTimeTag(false));$('time.in').each(replaceTimeTag(true));}
updateTimes();setInterval(updateTimes,60*1000);});$(document).ready(function(){var getReportsURLs=function(){var urls={};$('#report_select option').each(function(i,elem){urls[elem.value]={product:elem.dataset.urlProduct,version:elem.dataset.urlVersion,};});return urls;};var getURL=function(){var report=$('#report_select').val();var product=$('#products_select').val();var version=$('#product_version_select').val();var url;var reportsURLs=getReportsURLs();if(version==='Current Versions'){url=reportsURLs[report].product.replace('__PRODUCT__',product);}else{url=reportsURLs[report].version.replace('__PRODUCT__',product).replace('__VERSION__',version);}
return url;};$('.version-nav').on('change','select',function(){window.location=getURL();});$('.user-info-button').on('click',function(){$(this).siblings('.user-info-menu').toggle();});$(document).on('click',function(event){var userInfo=$('.user-info');if(!userInfo.is(event.target)&&!userInfo.has(event.target).length){$('.user-info-menu').hide();}});});$(document).ready(function(){var displayTelemetryEnvironment=(function(){var once=false;return function inner(){if(once){return;}
once=true;var container=$('#telemetryenvironment-json');if(container.length){var jsonData=container.data('telemetryenvironment');try{$('#telemetryenvironment-json').JSONView(jsonData);}catch(ex){console.warn('The data in the #telemetryenvironment-json dataset is not valid JSON');container.append($('<p>Error when parsing JSON. Showing raw value:</p>'));container.append(document.createTextNode(jsonData));}}};})();var displayMinidumpStackwalkJson=(function(){var once=false;return function inner(){if(once){return;}
once=true;var container=$('#minidump-stackwalk-json');if(container.length){var jsonData=container.data('minidumpstackwalk');try{$('#minidump-stackwalk-json').JSONView(jsonData);}catch(ex){console.warn('The data in the #minidump-stackwalk-json dataset is not valid JSON');container.append($('<p>Error when parsing JSON. Showing raw value:</p>'));container.append(document.createTextNode(jsonData));}}};})();var activeTab=0;if(location.hash.search(/#tab-/)>-1){var tabId=location.hash.split('#tab-')[1];var tab=$('#'+tabId);if(tab.length){if(tabId==='telemetryenvironment'){displayTelemetryEnvironment();}else if(tabId==='rawdump'){displayMinidumpStackwalkJson();}
$('.ui-tabs li a').each(function(i,tab){if(tab.href.search('#'+tabId)>-1){activeTab=i;}});}}
$('#report-index').tabs({active:activeTab,activate:function(event,ui){var tabId=ui.newPanel.attr('id');if(tabId==='telemetryenvironment'){displayTelemetryEnvironment();}else if(tabId==='rawdump'){displayMinidumpStackwalkJson();}
document.location.hash='tab-'+tabId;},});$('a[href="#allthreads"]').on('click',function(){var element=$(this);$('#allthreads').toggle(400);if(element.text()===element.data('show')){element.text(element.data('hide'));return true;}else{element.text(element.data('show'));location.hash='frames';return false;}});var tbls=$('#frames').find('table');var addExpand=function(sigColumn){$(sigColumn).append(' <a class="expand" href="#">[Expand]</a>');$('.expand').click(function(event){event.preventDefault();$('td:nth-child(3)',$(this).parents('tbody')).each(function(){$(this).text($(this).attr('title')).removeAttr('title');});$(this).remove();});};tbls.each(function(){var isExpandAdded=false;var cells=$(this).find('tbody tr td:nth-child(3)');cells.each(function(){if($(this).attr('title').length!==$(this).text().length&&!isExpandAdded){addExpand($(this).parents('tbody').find('th.signature-column'));isExpandAdded=true;}});});$('#modules-list').tablesorter({sortList:[[1,0]],headers:{1:{sorter:'digit'}}});var container=$('#mainbody');var signature=container.data('signature');var channel=container.data('channel');var product=container.data('product');window.correlations.getCorrelations(signature,channel,product).then(function(results){$('#correlation h3').text('Correlations for '+product+' '+channel[0].toUpperCase()+channel.substr(1));if(!Array.isArray(results)){$('#correlation pre').text('No correlation data available.');return;}
var content=results.join('\n');$('#correlation pre').empty().text(content);});BugLinks.enhance();});